          ...
          {payload.map((item, index) => {
            ...
            return (
              <div key={item.dataKey} className="flex w-full flex-wrap items-stretch gap-2">
                <div className="shrink-0 rounded-[2px] border border-border bg-background" style={{ backgroundColor: indicatorColor }} />
                <div className="flex flex-1 justify-between">
                  <span className="text-muted-foreground">{itemConfig?.label || item.name}</span>
                  {item.value && <span className="font-mono font-medium tabular-nums">{item.value.toLocaleString()}</span>}
                </div>
              </div>
            );
          })}
